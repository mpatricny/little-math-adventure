...
ACTION_WINDOWS = {
    "attack": {"start": 8, "end": 32},
    "defend": {"start": 4, "end": 28},
    "defeat": {"start": 4, "end": 28},
}
...
def despill(source: Image.Image, model_alpha_u8: np.ndarray) -> Image.Image:
    ...
    corrected_rgb = corrected_rgb * (1.0 - spill_weight) + neutral_rgb * spill_weight
...
def process_action(action: str, window: dict[str, int]) -> dict[str, object]:
    ...
    for source_index, video_frame in enumerate(container.decode(stream)):
        ...
        keyed = despill(source, protected_alpha)
        keyed.save(
    ...
    if len(frames) != 7:
        raise RuntimeError(f"Expected 7 {action} frames, got {len(frames)}")
...
def main() -> None:
    metadata = {
        "pipeline": "AutoSprite V3 CorridorKey alpha + green despill + source RGB",
    ...
    metadata_path = ROOT / "autosprite3-combat-every4.json"
