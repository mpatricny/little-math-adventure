...
**4 → 8 → 16 → 24 sekund → vypnuto.** Dvě správné první odpovědi posunou
čekání o stupeň výše. **Každá chybná první odpověď vrátí jeden stupeň**, nejníže
na 4 sekundy. Správné odpovědi s připomenutím také dovolí podporu ubírat;
rychlost odpovědi tuto změnu neřídí.

Tlamy se zobrazují nad všemi třemi čistými volbami současně, bez vazby na
správný výsledek. Místo pro ně je rezervované, takže obsah neposkočí. Pomoc
zůstane do volby. Odpověď před uplynutím čekání zůstává samostatná; `assisted`
se zapisuje až při skutečném zobrazení. Podpořená odpověď nepřidává rychlostní
bonus ani samostatné zvládnutí. Zkouška automatickou podporu nepoužívá.
...
- V patičce tabule je samostatný host `bonus` pro `⚡ +1` / `⚡ +2` za rychlé
  řešení známého příkladu. Jde o přírůstek do dosavadního ukazatele pod životy;
  naplnění čtyř políček přidá dosavadní bod poškození do celkového útoku.
  Podpořená odpověď nic nenabíjí. Úvodní obrázkové úlohy bez mastery klíče
  ani diagnostický vstup tento rychlostní bonus nepřidávají.
- Štít používá stejnou tabuli s titulkem „Braň se“, jedním známým příkladem,
...
- `introVersionSeen`, `hintLevel` a `hintCorrectStreak` jsou doplněné při
  hydrataci starších savů. Zachovávají se stabilní ID etap, historie, arithmetic
