...
Runtime source of truth for exam lengths and thresholds: `src/types/index.ts` (`EXAM_CONFIGS`). Problem composition is generated by `src/systems/MasterySystem.ts`. Content below **Archived legacy design** is historical and must not be used for current balancing.

---

## 0. Current exam balance

| Exam type | Location | Items | Result threshold | Composition |
|---|---|---:|---|---|
| **Sub-atom exam** | Guild | 8 | Bronze `5/8`, Silver `6/8`, Gold `7/8` | 3 result-unknown, 3 missing-part, 2 equation-vs-number |
| **Fluency Challenge** | Catacombs | 10 | Pass `8/10` | 4 result-unknown, 3 missing-part, 3 equation-vs-number |
| **Mastery Challenge** | Catacombs | 10 | Pass `9/10` | 3 result-unknown, 3 missing-part, 2 equation-vs-number, 2 equation-vs-equation |
| **Band gate exam** | Guild | 12 | Bronze `8/12`, Silver `10/12`, Gold `11/12` | 3 items from each sub-atom: one result, one missing-part, one equation-vs-number |
| **Band Mastery Challenge** | Guild | 14 | Pass `12/14` | 3 items from each sub-atom, plus 2 equation-vs-equation items from sub-atoms 3 and 4 |

Guild medal exams are scored only by the number of correct answers. Response time is still recorded for learning analytics but never changes the medal. Catacomb challenges use a hard 15-second limit and are described in Section 7.
...
- after enough practice the player can take an 8-item sub-atom exam,
...
### Fluent
Band is already `Secure`, and at least three of its four sub-atoms are `Fluent` or `Mastery`.

### Mastery
Band is `Fluent`, all four sub-atoms are at least `Fluent`, at least three sub-atoms are `Mastery`, and the Band Mastery Challenge is passed.
...
- a correct recorded response to the displayed problem,
- optionally assisted by a hint in lower-level battles.

Every displayed problem records one attempt. Any correct attempt increments `successful_solves`, including a correct answer taking more than 15 seconds. A correct answer over 15 seconds is also added to the slow-practice pool; an answer over 20 seconds remains correct for accuracy but its response time is excluded from RT calculations.

`successful_solves` and the rolling attempt history are used toward:

- the `20 successful solves` needed to unlock the sub-atom exam,
- the later Fluency and Mastery Challenge eligibility thresholds,
- rolling speed and accuracy thresholds.

The system stores attempts per character with sequence index, timestamp, correctness, response time, sub-atom, form, and context.
...
Speed is tracked by the system for learning analytics, but it does not affect Guild exam medals. Pass/fail challenges use correctness thresholds; the Catacomb scene separately treats a 15-second timeout as a wrong answer.

Speed is also used for:
...
Speed affects hidden mastery state, but not Guild exam medals.
...
- a Bronze result when the correctness threshold is met,
...
## 6. Exact medal thresholds for an 8-item sub-atom exam

Every sub-atom exam has:

- `8 items`
- no time limit for medals
...
No response-time threshold is used for medals. Bronze, Silver, and Gold all use the total number of correct answers.
...
Gold directly promotes a `Training` sub-atom to `Fluent`; Bronze and Silver promote it to `Secure`. No sub-atom exam directly grants `Mastery`.

The medal also determines:
...
- `10 items`
- `15 seconds per item`
- balanced across the three forms learned before mastery
- a fixed queue with no replacement problem after a wrong answer or timeout
- `3 lives`, because three wrong answers make `8/10` unreachable

Pass condition:

- `correct >= 8/10`

The challenge is passed after the fixed queue with at least `8` correct answers. A wrong answer or timeout removes one life; the challenge ends in defeat once all three lives are gone. Response time is still recorded for analytics.
...
- a fixed queue with no replacement problem after a wrong answer or timeout
- `2 lives`, because two wrong answers make `9/10` unreachable

Pass condition:

- `correct >= 9/10`

The challenge is passed after the fixed queue with at least `9` correct answers. A wrong answer or timeout removes one life; the challenge ends in defeat once both lives are gone. Response time is still recorded for analytics.
...
This transition is automatic when the sub-atom condition is reached. The band gate medal does not promote the band directly beyond `Secure`.
...
There is no separate Band Fluency Challenge. Band fluency is derived automatically from the states of its sub-atoms.
...
- includes `3` items from every sub-atom
- adds `2` advanced equation-vs-equation comparisons from the three-operand and mixed sub-atoms
...
- `3` items from each of the band's four sub-atoms
- every sub-atom contributes one result, one missing-part, and one comparison item

### 9.2 Gate medal rules

- **Gold** = `correct >= 11/12`
- **Silver** = `correct = 10/12`
- **Bronze** = `correct = 8/12 or 9/12`
- **Fail** = `correct <= 7/12`

No response-time threshold is used for gate medals. Bronze, Silver, and Gold all use the total number of correct answers.
...
- Bronze, Silver, or Gold = band becomes `Secure`
- the band later becomes `Fluent` automatically when at least three sub-atoms are `Fluent` or `Mastery` (8.3)
...
## 11. Exact 8-item exam composition for all sub-atoms

## 11.1 All sub-atom exams

- 3 result-unknown
- 3 missing-part
- 2 compare equation vs number

Equation-vs-equation is intentionally reserved for mastery work and does not appear in the standard sub-atom exam.
...
4. Bronze or Silver makes the sub-atom `Secure`; Gold makes it `Fluent`
...
9. Bronze or better on the gate makes the band `Secure` and unlocks the next band
...
# Archived legacy design (not active)

> Historical design retained for reference only. The values below are obsolete; use the active specification above and `EXAM_CONFIGS` for current behavior.
