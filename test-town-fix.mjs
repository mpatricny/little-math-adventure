...
await page.evaluate(() => {
    ...
    localStorage.setItem('littleMathAdventure_slot_1', JSON.stringify({
        player: makePlayer('Tomáš', 'boy_knight', 3, 'pet_slime'),
        mathStats: makeStats(), timestamp: Date.now()
    }));
});
await page.reload();
...
// Enter arena + start battle
await canvas.click({ position: { x: 108 * sx, y: 600 * sy } });
await page.waitForTimeout(8000);
await canvas.click({ position: { x: 890 * sx, y: 655 * sy } });
await page.waitForTimeout(3000);

// Battle — check hero positioning
await page.screenshot({ path: '/tmp/final-01-battle.png' });

// Click ÚTOK — math board should show player name
await canvas.click({ position: { x: 640 * sx, y: 660 * sy } });
await page.waitForTimeout(2000);
await page.screenshot({ path: '/tmp/final-02-math-with-name.png' });
